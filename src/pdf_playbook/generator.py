...
def get_available_ai_engines() -> dict[str, str]:
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=1.5) as res:
            ...
            if models:
                engines["ollama"] = f"Local Ollama (Models: {', '.join(models[:3])})"
            else:
                engines["ollama"] = "Local Ollama (Running)"
    except Exception:
        pass
...
def _synthesize_contextual_pages(title: str, audience: str, topic: str) -> list[tuple[str, str, list[tuple[str, str]]]]:
    ...
    Dynamically generates an 18-page structured curriculum specifically tailored to the given topic and audience.
    """
    t_clean = title.strip() or "Engineering Playbook"
    a_clean = audience.strip() or "Developers"
    p_clean = topic.strip() or "Master your workflow"
    ...
    if is_video_streaming and is_kids:
        return [
            ...
            ("Getting Started & Navigation", "Master the screen layout, buttons, and finding what you want.", [
                ...
                ("Voice Search", "Tap the microphone icon and speak clearly to find videos without typing."),
            ...
            ("The Video Player Controls", "Take full control of playback, sound, and screen size.", [
                ...
                ("Replay & Skip", "Double-tap the right side of the screen to jump ahead 10 seconds, or the left to rewind."),
                ("Volume & Mute", "Use your device volume rocker or speaker slider to keep audio comfortable and clear.")
            ]),
            ...
            ("Voice Search & Audio Features", "Use voice commands to find videos fast.", [
                ("Speaking Clearly", "Hold down the mic button and say show titles clearly like 'National Geographic Kids'."),
            ...
            ("Privacy & Online Safety Rules", "Essential rules to protect yourself and your family.", [
                ...
                ("Reporting Bad Videos", "Tap the three dots on any bad video and select 'Report' to notify safety teams."),
    ...
    # Universal General Synthesizer for any custom technical topic
    return [
...
def fetch_ai_playbook_pages(
    ...
    *,
    ...
    """
    Generate 18 meaningful, custom chapters specifically for the topic using AI or dynamic contextual synthesizer.
    """
    # 1. Check Gemini API
...
Return raw JSON only."""
    ...
    # Fallback to intelligent contextual topic synthesizer (offline & testing)
    pages = _synthesize_contextual_pages(title, audience, topic)
    return pages, "AgentBroko Dynamic Synthesizer"
...
def generate_playbook(
    ...
    *,
    ...
    if spec_file and Path(spec_file).exists():
        ...
        # 2. Dynamic AI Generation Mode (Gemini / OpenAI / Ollama / Dynamic Synthesizer)
